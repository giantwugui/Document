...
* **位置独立。**消息客户端不必知道谁会处理他们的消息，或者服务的位置在哪儿。客户端只需要了解通过哪个队列或主题来发送消息。因此，只要服务呢个够从队列或主题中获取消息即可。
* **确保投递。**发送异步消息，客户端完全可以详细消息会被投递。即使在消息方式时，服务无法是哟共，消息也会被存储起来，知道服务重新可以是哟共为止。
...
![image-20200710163425459](C:\Users\wanxin\AppData\Roaming\Typora\typora-user-images\image-20200710163425459.png)

![image-20200710163534356](C:\Users\wanxin\AppData\Roaming\Typora\typora-user-images\image-20200710163534356.png)
...
如果一斤存在该名称的队列或煮汤，就会使用已有的。如果为存在，则会创建一个新的目的地(通常是队列)。如果要制定创建的目的地类型的化，可以将之前创建的队列或目的地的bean装配起来
...
JmsTemplate提供了convertAndSend()方法。convertAndSend不需要MessageCreator作为参数。因为convertAndSend会是哟共内置的消息转换器创建消息
...
![image-20200710165103033](C:\Users\wanxin\AppData\Roaming\Typora\typora-user-images\image-20200710165103033.png)
...
![image-20200713113704589](C:\Users\wanxin\AppData\Roaming\Typora\typora-user-images\image-20200713113704589.png)
